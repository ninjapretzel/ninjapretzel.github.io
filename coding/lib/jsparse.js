// from https://dxr.mozilla.org/mozilla/source/js/narcissus/jsparse.js
// modified to work with firefox 71
...
Tokenizer.prototype = {
	...
	get: function () {
		...
		for (;;) {
			...
			var match = (this.scanNewlines ? whitespaceRegExp : linestartRegExp).exec(input);
			if (match) {
				var spaces = match[0];
...
function Script(tokenizer, compiler) {
	var node = Statements(tokenizer, compiler);
	node.type = SCRIPT;
	node.funDecls = compiler.funDecls;
	node.varDecls = compiler.varDecls;
	return node;
}
...
Object.defineProperty(Array.prototype,
	'top',
	{
		value: function () { return this.length && this[this.length-1]; },
		writable: false, 
		enumerable: false, 
		configurable: false,
	}
...
Np.__defineGetter__('filename', function() { return this.tokenizer.filename; });
	...
	while ((tt = t.get()) != END) {
		...
		switch (tt) {
		  ...
		  case RIGHT_PAREN:
			...
			while ((tt = reduce().type) != GROUP && tt != CALL && tt != NEW_WITH_ARGS) {
				continue;
			}
			if (tt != GROUP) {
				n = operands.top();
...
function parse(source, filename, lineNumber) {
	var tokenizer = new Tokenizer(source, filename, lineNumber);
	var compiler = new CompilerContext(false);
	var script = Script(tokenizer, compiler);
	if (!tokenizer.done) {
